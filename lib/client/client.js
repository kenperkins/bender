...
var core = {
    ...
    runPuppet: function(sshConnection, callback) {
        callback();
    },

    bootstrapServer: function(serverId, callback) {
        var self = this;
        ...
        self.getServer(serverId, function(err, server) {
            ...
            var puppet = new Connection();

            async.parallel([
                connectSshToServer(srv, server.getPrivateIp()),
                connectSshToServer(puppet, 'puppet.chicago.il.private.mgmt.clpbrd.com'),
            ],
                function(err) {

                    if (err) {
                        self.log.error('Unable to establish ssh connections', err);
                        callback(err);
                        return;
                    }

                    async.parallel([
                        setPuppetMasterConfig,
                        setResolvConfDomain,
                        updateIpWhitelistOnServer,
                        updateIpWhitelistOnPupppetMaster,
                        function(asyncCallback) {
                            self.runPuppet(srv, asyncCallback);
                        }
                    ],
                        function(err) {
                            callback();
            ...
            function setPuppetMasterConfig(asyncCallback) {
                asyncCallback();
            }

            function setResolvConfDomain(asyncCallback) {
                server.environment.getPrivateDomain().done(function(err, domain) {
                    if (err) {
                        self.log.error('Unable to get private domain', err);
                        asyncCallback(err);
                        return;
                    }

                    var cmd = 'echo -e \"domain ' + domain.name + '\" >> /etc/resolv.conf';

                    self.log.verbose('Running ssh command', cmd);

                    srv.exec(cmd, function(err, stream) {
                        if (err) {
            ...
            function updateIpWhitelistOnServer(asyncCallback) {
                asyncCallback();
            }

            function updateIpWhitelistOnPupppetMaster(asyncCallback) {
                asyncCallback();
            }
        });

        function connectSshToServer(connect, host) {
            return function(asyncCallback) {

                var hasCalledBack = false;

                connect.on('ready', function() {
                    self.log.verbose('Connection :: ready', host);
                ...
                connect.on('end', function() {
                    self.log.verbose('Connection :: end');
                });

                connect.on('close', function(had_error) {
                    self.log.verbose('Connection :: close');
                    self.log.verbose('Connection :: close :: had_error: ' + had_error);
                });

                connect.on('keyboard-interactive', function() {
                    self.log.verbose('Connection :: keyboard-interactive');
                });
