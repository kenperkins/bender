...
var core = {
            ...
            dbServer.save().done(function(err, result) {
                ...
                async.parallel([
                    ...
                    function(err) {
                        ...
                        self.installBender(dbServer.id, function(err) {
    ...
    execServerCommand: function(serverId, command, callback) {
        var self = this,
            cmd = 'bender --log debug --server ' + serverId + ' --command ' + command;

        console.dir(typeof(serverId));
    ...
    _execSshCommand: function(options, callback) {
        ...
        connect.on('ready', function() {
            self.log.verbose('bender-server :: connected : ' + options.label);
            self.log.verbose('bender-server :: exec : ' + options.label, options.command);
        ...
        connect.on('error', function(err) {
            self.log.verbose('bender-server :: error : ' + options.label, err);
        ...
        connect.on('end', function() {
            self.log.verbose('bender-server :: end : ' + options.label);
        });

        connect.on('close', function(had_error) {
            self.log.verbose('bender-server :: close : ' + options.label, had_error);
        });
            ...
            async.series([
                ...
                function(asyncCallback) {
                    ...
                    scp.send(options, function(err) {
                        if (err) {
                            self.log.error('Unable to set bender config', err);
                        }
