...
/**
 * @name exports.createClient
 * @description Static factory to creating and initializing
 * an instance of the bender client
 *
...
/**
 * @name exports.Client
 * @description Constructor function for the Bender Client
 * @constructor
 * @this {Client}
 * @param {object} options
 */
...
var core = {
    ...
    /**
     * Initializes the bender client with the provided options. This includes
     * connecting to sequelize, rackspace, and starting the logging
     *
     * @param {object} options
     * @param {Function} callback
     */
    ...
    /**
     * Create an environment within bender
     *
    ...
    createEnvironment: function(details, callback) {
        ...
        // Instantiate the Sequelize model
        ...
        env
            ...
            .done(function(err, result) {
                ...
                // Set the public and private domain models on the environment
                async.parallel([
                    function(asyncCallback) {
                        env
                    ...
                    if (err) {
                        ...
                        // TODO open question on what to do here, as we've
                        // already saved the environment
                        callback(err);
                        return;
                    }
    ...
    /**
     * Create a service within bender
     *
     * @param {object} details
     * @param {Function} callback
     */
    ...
    /**
     * Get the list of current environments from bender
     *
     * @param {Function} callback
     */
    ...
    /**
     * Get the list of services from bender
     *
     * @param {Function} callback
     */
    ...
    /**
     * Get the list of providers within bender
     * TODO: Figure out a system which allows or multiple providers simultaneously
     * @param {Function} callback
     */
    ...
    /**
     * Create a new provider within bender
     *
     * @param {String} name
     * @param {Function} callback
     */
    ...
    /**
     * Get a server with a provided server id from bender
     * Eager loading 'Environment' and 'Addresses' as part of the query
     *  
    ...
    /**
     * Get all of the servers from bender
     * Eager loading 'Environment' and 'Addresses' as part of the query
     * 
     * @param {Function} callback
     */
    ...
    /**
     * Get an array of servers for a provided environment
     * Eager loading 'Environment' and 'Addresses' as part of the query
     *
     * @param {Number|Environment} environment
     * @param callback
     */
    getServersForEnvironment: function(environment, callback) {
        var self = this;

        var environmentId = environment;
    ...
    /**
     * Find all of the servers for a given service and environment.
     *
     * @param {Number|Environment} environment
     * @param {Number|Service} service
     * @param {Function} callback
     */
    getServersForService: function(environment, service, callback) {
        var self = this;
        ...
        if (typeof(service) === 'object') {
            getServersForService(service);
        }
        else {
            self.Service.find(service).done(function(err, service) {
                if (err) {
        ...
        function getServersForService(service) {
    ...
    /**
     * Create a new server with the current provider
     *
     * @description This function creates a new instance with the current provider,
     * but does not add the instance to bender. Due to the potential for extremely
     * long "active" times on new instances, it's better to fully decouple create
     * from addition/provision within bender.
     *
     * @param {object} details
     * @param {Function} callback
     */
    ...
    /**
     * Add an already created server to bender.
     * @description If the current server is not in the "Active" status, it will
     * not be added to bender.
     *
     * @param {object} details
     * @param {Environment} details.environment The environment of for the server
     * @param {object} details.server The server to add
     * @param {Array} [details.services] A list of services to map for the server
     * @param {String} [details.drive] The drive type for the VM
     * @param {Number} [details.driveSize] The size of the drive, in GB (if Block device)
     * @param {Function} callback
     */
    addServer: function(details, callback) {
        ...
        self.rackspace.getServer(details.server.id, function(err, server) {
            ...
            // Build the model
            var benderServer = self.Server.build({
                name: server.name,
            ...
            benderServer.save().done(function(err, result) {
                if (err) {
                ...
                // Once we've created the bender server, we need to create
                // and associate the ipaddresses for the server

                // We can also set the services for the new server
                self.log.verbose('Local Server created, creating ip address records');
                async.parallel([
                    saveIpAddress(addresses, benderServer, server.addresses.public, true),
                    saveIpAddress(addresses, benderServer, server.addresses.private, false),
                    function(asyncCallback) {
                        benderServer.setServices(details.services).done(asyncCallback);
                    }],
                    function(err) {

                        // Wire up the values for the return object
                        benderServer.addresses = addresses;
                        benderServer.environment = details.environment;
                        ...
                        // If we're doing a block device, we need to create and
                        ...
                        // If not doing a block device, lets just create the data dir
                        // and continue
                        else {
                            self._execSshCommand({
                                label: benderServer.getFriendlyLabel(),
                                host: benderServer.getPrivateIp(),
                        ...
                        function provision() {
                            // Provision the server after adding
                            self.provisionServer(benderServer, function(err) {
                                callback(err, {
                                    server: benderServer,
    ...
    /**
     * Provision a server with bender
     * @descirption provisioning includes installing bender, updating
     * the global whitelists, and finally authenticating the server and running
     * puppet on the target server
     *
     * @param {Server|Number} server The server or serverId to provision
     * @param {Function} callback
     */
    provisionServer: function(server, callback) {
        ...
        self.installBender(server, function(err) {
            ...
            self.updateServerWhitelists(function(err) {
                ...
                self.authenticateServerOnPuppet(server, callback);
            });
        });
    },
     ...
     * @description This function creates a block storage volume with the current
     * provider and then attaches the volume to the server
     *
     * @param {object} options
     * @param {Server} options.server The server to attach the volume to
     * @param {String} options.type The type of block device to create
     * @param {Number} options.size The size (in GB) of the device
     * @param {String} [options.device=/dev/xvdb] The device to mount to
     * @param {Number} [options.maxWait=1800] The max time to wait when creating
     * the volume
     * @param {Function} [options.onUpdate] A function to call on an interval during
     * volume creation
     * @param {Function} [options.onComplete] A function to call upon completion of
     * the volumes creation
     * @param {Function} callback
     */
    createAndAttachVolume: function(options, callback) {
        var self = this;

        options.device = options.device || '/dev/xvdb';
        options.maxWait = options.maxWait || 1800;
                    ...
                    async.series([
                        function(asyncCallback) {
                            self._execSshCommand({
                                ...
                                command: 'sfdisk ' + options.device + ' <<< ",,83"'
                            }, asyncCallback);
                        },
                        function(asyncCallback) {
                            self._execSshCommand({
                                label: options.server.getFriendlyLabel(),
                                host: options.server.getPrivateIp(),
                                command: 'mkfs -t ext4 ' + options.device + '1'
                            }, asyncCallback);
                        }], function(err) {
                        if (err) {
