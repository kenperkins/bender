...
var core = {
            ...
            dbServer.save().done(function(err, result) {
                ...
                async.parallel([
                    ...
                    function(err) {
                        ...
                        if (options.drive === 'Block SATA' || options.drive === 'Block SSD') {
                            self.createAndAttachVolume({
                                server: dbServer,
                                size: options.driveSize,
                                type: options.drive
                            }, function(err, volume) {
                                provision();
                            });
    ...
    provisionServer: function(server, callback) {
        ...
        self.installBender(server, '/usr/lib/node_modules/bender/lib/config.json', function(err) {
                    ...
                    async.series([
                        ...
                        function(asyncCallback) {
                            self._execSshCommand({
                                label: options.server.getFriendlyLabel(),
                                host: options.server.getPrivateIp(),
                                command: 'mount /dev/xvdb1  /usr/local/data'
                            }, asyncCallback);
                        },
                    ], function(err) {
                        callback(err, volume);
    ...
    updateEnvironment: function(envId, callback) {
        var self = this;

        self.getServersForEnvironment(envId, function(err, servers) {
            if (err) {
            ...
            async.forEach(servers, function(server, next) {
                self.log.verbose('Updating ' + server.getFriendlyLabel());
