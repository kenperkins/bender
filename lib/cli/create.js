...
exports.Create = {
        ...
        async.series([
            ...
            function(callback) {
                self.bender.createServer(options, function(err, result) {
                    ...
                    if (result) {
                        console.log('  Successfully created server '.cyan + result.name.green);
                    }
    ...
    addServer: function() {
        ...
        async.series([
            function(callback) {
                console.log('\n  Please select existing server to provision:'.bold.yellow);

                self.bender.getUnprovisionedServers(function(err, servers) {
                    console.log('\n  Please choose an existing server to add:'.bold.yellow);
                    console.log('    to bender.'.data);
                    self.app.choose(_.map(servers, function(server) {
                        return server.name.yellow;
                    }), function(i) {
                        options.server = servers[i];
                        callback();
                    });
                });
            },
            ...
            function(callback) {
                ...
                self.bender.getServices(function(err, services) {
                    ...
                    function addService() {
                        ...
                        self.app.choose(serviceOptions, function(i) {
            ...
            function(callback) {
                self.bender.addServer(options, function(err, result) {

                    if (err) {
                        console.log('Errors during server addition!'.red, err);
                    }

                    if (result) {
                        console.log('  Successfully added server '.cyan + result.server.name.green);
                    }
        ...
        async.series([
            ...
            function(callback) {
                ...
                self.bender.getServices(function(err, services) {
                    ...
                    _.each(choices.server.services, function(serverService) {
                        console.log('\t' + serverService.name);

                        _.each(services, function(service, index) {
                            if (service.name === serverService.name) {
                                services.splice(index, 1);
                            }
                        });
                    });

                    console.log('\n  Please add services to this server:'.bold.yellow);
                        ...
                        self.app.choose(serviceOptions, function(i) {
                            ...
                            _.each(services, function(realService, index) {
                                if (service === realService.name) {
                                    ...
                                    choices.services.push(realService);
            ...
            function(err) {
                ...
                choices.server.setServices(choices.services).done(function(err) {
