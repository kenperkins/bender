...
exports.List = {
        ...
        self.bender.getServers(function(err, servers) {
            ...
            _.each(servers, function(server) {
                ...
                _.each(server.addresses, function(address) {
                    ...
                    else if (!address.isPublic && address.type == '4') {
    ...
    listUnprovisonedServers: function(callback) {
        var self = this;
        console.log('Loading Unprovisioned Servers...\n'.cyan);
        self.bender.getUnprovisionedServers(function(err, servers) {

            if (!servers.length) {
                console.log('\n  No Unprovisioned Servers Found'.red);
                callback && callback();
                return;
            }
