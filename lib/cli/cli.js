...
var CommandLine = exports.CommandLine = function(config) {
    ...
    self.app
        ...
        .option('--log <level>', 'Configure log level. Options are [debug|versbose|info|warn|error]')
        .option('--whitelist <interface>', 'Whitelist all servers on the target interface')
        .option('--bootstrap', 'Bootstrap the selected server')
    ...
    if (self.app.resetDatabase) {
        ...
        self.initialize(function() {
            self.whiteListServers(self.app.whitelist);
        });
    }
    else if (self.app.bootstrap) {
        self.initialize(function() {
