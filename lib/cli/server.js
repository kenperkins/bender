...
exports.Server = {
   ...
   bootstrap: function(serverId) {
       ...
       self.bender.getServer(serverId, function(err, server) {
           ...
           server.environment.getPrivateDomain().done(function(err, privateDomain) {
               ...
               self._checkFileForValue('/etc/puppet/puppet.conf', 'server=', 'puppet.chicago.il.private.test.clpbrd.com');
               self._checkFileForValue('/etc/puppet/puppet.conf', 'environment=', server.environment.name);
           });
       });
   },
   ...
   _checkFileForValue: function(file, prefix, newValue) {
       var self = this,
           resolv = fs.readFileSync(file).toString().split('\n'),
           output = '',
           found = false;
