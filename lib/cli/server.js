   ...
   bootstrap: function(serverId) {
       ...
       self.bender.getServer(serverId, function(err, server) {
           ...
           server.environment.getPrivateDomain().done(function(err, privateDomain) {
               ...
               self._checkFileForValue('/etc/resolv.conf', 'domain ', privateDomain.name);
               self._checkFileForValue('/etc/puppet/puppet.conf', 'server=', 'puppet.chicago.il.private.mgmt.clpbrd.com', '[main]');
               self._checkFileForValue('/etc/puppet/puppet.conf', 'environment=', server.environment.name, '[main]');

               // TODO hookup whitelist for all servers
               // TODO hookup puppet run
           });
       });
   },
