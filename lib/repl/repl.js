...
var core = {
    ...
    deploy: function(callback) {
        ...
        console.log('Are you sure you want to deploy?\n'.bold.green);

        program.choose(['Yes', 'No'], function(i) {
            if (i === 1) {
                callback();
                return;
            }
                            ...
                            async.forEachSeries(servers, function(server, next) {
                                ...
                                async.series([
                                    ...
                                    function(asyncCallback) {
                                        ...
                                        // Rolling deployment, update one pool of servers, then the others
                                        var poolA = _.filter(appServers, function(server, index) {
                                            return index % 2 === 0;
                                        });
                                        var poolB = _.filter(appServers, function(server, index) {
                                                return index % 2 !== 0;
                                        });
                                            
                                        var loadBalancerId = self.config.loadBalancers[self.currentEnvironment.name];
                                        if (!loadBalancerId) {
                                            //TODO: Just deploy as normal
                                        }
                                        
                                        self.bender.getServer(loadBalancerId, function(err, loadBalancer) {
                                            if (err) {
                                                console.error('Unable to get load balancer ' + loadBalancerId);
                                                asyncCallback(err);
                                                return;
                                            }

                                            // swap out pool A from the lb
                                            swapPools(loadBalancer, [], poolA, true, function(err) {
                                                if (err) {
                                                    asyncCallback(err);
                                                    return;
                                                }

                                                // deploy the bits to poolA
                                                async.forEach(
                                                    poolA,
                                                    function(appServer, callback) {
                                                        deployServer(appServer, true, function(err) {
                                                            if (err) {
                                                                //TODO: Log
                                                                callback(err);
                                                                return;
                                                            ...
                                                            // add back app servers, remove other pool of servers
                                                            swapPools(loadBalancer, poolA, poolB, true, function(err) {
                                                                if (err) {
                                                                    //TODO: Log
                                                                    callback(err);
                                                                    return;
                                                                ...
                                                                async.forEach(
                                                                    ...
                                                                    function(appServer, callback) {
                                                                        deployServer(appServer, true, function(err) {
                                                                            ...
                                                                            // swap back in poolB
                                                                            swapPools(loadBalancer, poolB, [], true, function(err) {
                                                                                callback(err);
                                                                            });
                                                                        });
                                                                    },
                                        ...
                                        function swapPools(loadBalancer, swapIn, swapOut, reloadOnCompletion, callback) {
                                            console.log('\nStarting Rotation on '.green + loadBalancer.getFriendlyLabel().yellow);
                                            console.log('  Reloading on completion?: '.yellow + reloadOnCompletion);

                                            swapIn = swapIn || [];
                                            swapOut = swapOut || [];

                                            if (swapIn.length) {
                                                console.log('  <<<< Swapping In Servers'.bold.green);
                                                console.log('       Servers: '.yellow + _.map(swapIn, function(appServer) {
                                                    return appServer.server.name.green;
                                                }).join(', '));
                                            }

                                            if (swapOut.length) {
                                                console.log('  >>>> Swapping Out Servers'.bold.red);
                                                console.log('       Servers: '.yellow + _.map(swapOut,function(appServer) {
                                                    return appServer.server.name.red;
                                                }).join(', '));
                                            }
                                            ...
                                            async.forEachSeries(
                                                swapOut,
                                                function(appServer, callback) {
                                                    console.log('         Swapping Out: '.yellow + appServer.server.getFriendlyLabel().yellow);
                                                    self.bender.execServerCommand(
                                                        loadBalancer,
                                                        'swapout-' + appServer.server.id, 
                                                        function(err) {
                                                            if (err) {
                                                                console.log('Unable to swap out ' + appServer.server.getFriendlyLabel());
                                                            }
                                                            callback(err);
                                                        }
                                                    );
                                                ...
                                                function(err) {
                                                    ...
                                                    async.forEachSeries(
                                                        swapIn,
                                                        function(appServer, callback) {
                                                            console.log('         Swapping In: '.yellow + appServer.server.getFriendlyLabel().yellow);
                                                            self.bender.execServerCommand(
                                                                loadBalancer, 'swapin-' + appServer.server.id, function(err) {
                                                                    if (err) {
                                                                        console.log('Unable to swap in ' + 
                                                                                    appServer.server.getFriendlyLabel());
                                                                    }
                                                                    callback(err);
                                                                }
                                                            );
                                                        ...
                                                        function(err) {
                                                            ...
                                                            if (reloadOnCompletion) {
                                                                console.log('\n  Initiating reload on '.green + loadBalancer.getFriendlyLabel().yellow);
                                                                self.bender.execServiceCommand(
                                                                    loadBalancer, 'nginx', 'reload', function(err) {
                                    ...
                                    function(asyncCallback) {
                                        async.forEach(workers, function(worker, next) {
                                            console.log('Starting '.yellow + worker.service.name.green + ' on '.yellow + worker.server.name.yellow);
                                            self.bender.execServiceCommand(worker.server.id,
                                                worker.service,
                                                'start', next);
                                        }, asyncCallback);
                                    ...
                                    function(asyncCallback) {
                                        async.forEach(others, function(other, next) {
                                            console.log('Restarting '.yellow + other.service.name.green + ' on '.yellow + other.server.name.yellow);
        ...
        function deployServer(target, restartAfterDeploy, callback) {

            if (typeof restartAfterDeploy === 'function') {
                callback = restartAfterDeploy;
            ...
            console.log('Deploying to '.yellow + target.service.name.green +
            ...
            rsync.execute(function(err, stdout, stderr) {
                if (err) {
                    ...
                    console.log('  Deployed to '.yellow +
                        target.service.name.green +
                        ' ' + target.server.name.yellow);

                    if (restartAfterDeploy) {
                        console.log('  Restarting '.yellow + target.service.name.green + ' on '.yellow + target.server.name.yellow + '\n');
                        self.bender.execServiceCommand(
                            target.server,
                            target.service,
                            'restart', function(err, code) {
                                setTimeout(function() {
                                    callback(err);
                                }, 1000);
                            });
