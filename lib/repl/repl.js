...
var core = {
                            ...
                            async.series([
                                function(asyncCallback) {
                                    ...
                                    async.forEach(workers, function(worker, next) {
                                        console.log('Stopping ' + worker.service.name + ' ' + worker.server.name);
                                ...
                                function(asyncCallback) {
                                    ...
                                    async.forEach(app, self.deployServer, asyncCallback);
                                },
                                function(asyncCallback) {
                                    async.forEach(app, function(appServer, next) {
                                        console.log('Restarting ' + appServer.service.name + ' ' + appServer.server.name);
                                        self.bender.execServiceCommand(appServer.server.id,
                                            appServer.service.id,
                                            'restart', next);
                                    }, asyncCallback);
                                },
                                function(asyncCallback) {
                                    // deploy the bits to the workers servers
                                    async.forEach(workers, self.deployServer, asyncCallback);
                                },
                                function(asyncCallback) {
                                    // deploy the bits to the other servers
                                    async.forEach(others, self.deployServer, asyncCallback);
                                },
                                function(asyncCallback) {
                                    async.forEach(workers, function(worker, next) {
                                        console.log('Starting ' + worker.service.name + ' ' + worker.server.name);
                                        self.bender.execServiceCommand(worker.server.id,
                                            worker.service.id,
                                            'start', next);
                                    }, asyncCallback);
                                ...
                                function(asyncCallback) {
                                    async.forEach(others, function(other, next) {
                                        console.log('Restarting ' + other.service.name + ' ' + other.server.name);
    ...
    deployServer: function(target, callback) {
        console.log('Deploying to ' + target.service.name +
            ' ' + target.server.name);
        // Build the command
        var rsync = new Rsync()
            .shell('ssh')
